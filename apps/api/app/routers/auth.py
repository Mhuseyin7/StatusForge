...
from app.security import create_access_token, hash_password, new_refresh_token, token_hash, verify_password
...
def issue_tokens(db: Session, user: User, request: Request) -> TokenResponse:
    ...
    db.add(SessionToken(user_id=user.id, token_hash=token_hash(refresh), expires_at=datetime.now(timezone.utc) + timedelta(days=settings.refresh_token_days), user_agent=request.headers.get("user-agent", "")[:512], ip_address=request.client.host if request.client else None))
